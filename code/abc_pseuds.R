...
### pseuds ###
pseud_data <- read_csv("empirical/data/Pseudomys.csv") %>% as.data.frame()
pseud_tree <- read.tree("empirical/trees/Pseudomys.tre")

rownames(pseud_data) <- pseud_data$species

sortedData <- sortTraitData(phy = pseud_tree, y = pseud_data, data.name = "trait", log.trait = F)

pseud_age <- round(max(nodeHeights(pseud_tree)), 0)

pseud_root <- transformPhylo.ML(y = sortedData$trait, phy = sortedData$phy, model = "bm")$root.state

# estimate BM
pseud_bm <- transformPhylo.ML(y = sortedData$trait, phy = sortedData$phy, model = "bm")$brownianVariance[[1]]

# estimate OU
pseud_ou <- transformPhylo.ML(y = sortedData$trait, phy = sortedData$phy, model = "ou")$Alpha[[1]]

# estimate MC
pseud_mc <- abs(fit_t_comp(sortedData$phy, setNames(as.numeric(t(sortedData$trait)), rownames(sortedData$trait)), model = "MC")$s)
...
sim.vals <- tibble(lambda1 = sapply(round(abs(rnorm(100, r$b, sd = 0.01)), 3), max, 0.001),
                   ...
                   mubg = sapply(round(abs(rnorm(100, r$mu_1*0.1, sd = 0.01)), 3), max, 0.001),
                   muibg = sapply(round(abs(rnorm(100, r$mu_2, sd = 0.01)), 3), max, 0.001),
                   bm.vals = sapply(round(abs(rnorm(100, pseud_bm, sd = 0.05)), 3), max, 0.001),
                   mc.vals = sapply(round(abs(rnorm(100, pseud_mc, sd = 0.05)), 3), max, 0.001),
                   ou.vals = sapply(round(abs(rnorm(100, pseud_ou, sd = 0.05)), 3), max, 0.001)) %>%
  unique()
...
runCAMPSITE.ABC <- function(nsim, lambda1, tau0, mubg, muibg, bm, mc, ou, path){
  ...
  set.seed(42)
  sim <- foreach(i=1:nsim, .packages = c("ape"), .export = c("pseud_root", "pseud_age")) %dopar% {
    repeat {
      model <- sim_CAMPSITE(pars = c(lambda1,
                                     ...
                                     bm,
                                     20),
                            ou = list(c(pseud_root),
                                      c(ou),
                                      c(ou)),
                            bounds = c(-Inf,
                                       Inf),
                            root.value = pseud_root,
                            age.max = pseud_age,
                            step.size = 0.01,
                            full.sim = T,
                            plot = F
...
# simulate "BM":
mapply(runCAMPSITE.ABC, rep(100, 100), sim.vals$lambda1, sim.vals$tau0, sim.vals$mubg, sim.vals$muibg, sim.vals$bm.vals, rep(0, 100), rep(0, 100), rep("output/emp_sims/pseuds/", 100))

# simulate competition:
mapply(runCAMPSITE.ABC, rep(100, 100), sim.vals$lambda1, sim.vals$tau0, sim.vals$mubg, sim.vals$muibg, sim.vals$bm.vals, sim.vals$mc.vals, rep(0, 100), rep("output/emp_sims/pseuds/", 100))

# simulate selection:
mapply(runCAMPSITE.ABC, rep(100, 100), sim.vals$lambda1, sim.vals$tau0, sim.vals$mubg, sim.vals$muibg, sim.vals$bm.vals, rep(0, 100), sim.vals$ou.vals, rep("output/emp_sims/pseuds/", 100))

# simulate competition+selection:
mapply(runCAMPSITE.ABC, rep(100, 100), sim.vals$lambda1, sim.vals$tau0, sim.vals$mubg, sim.vals$muibg, sim.vals$bm.vals, sim.vals$mc.vals, sim.vals$ou.vals, rep("output/emp_sims/pseuds/", 100))

# summarise
models <- list.files("output/emp_sims/pseuds", full.names = T)
model.names <- list.files("output/emp_sims/pseuds", full.names = F)
...
traits_models %>% write_csv("output/summary_pseuds_empirical_models.csv")

traits_models <- read_csv("output/summary_pseuds_empirical_models.csv") %>% as.data.frame()
...
# model selection
stat.pseud <- data.frame(sd = sd(sortedData$trait),
...
set.seed(42)
modsel.pseud <- postpr(stat.pseud[1,],
                         models,
                         traits,
                         tol=.2,
                         method="mnlogistic")

summary(modsel.pseud)

# goodness-of-fit

set.seed(42)
pseud.gfit.selec = gfit(target=stat.pseud[1,],
                          sumstat=traits[models=="selec",],
                          statistic=mean,
                          nb.replicate=100)

summary(pseud.gfit.selec)
plot(pseud.gfit.selec, main="Histogram under H0; Selection")

set.seed(42)
pseud.gfit.bm = gfit(target=stat.pseud[1,],
                       sumstat=traits[models=="bm",],
                       statistic=mean,
                       nb.replicate=100)

summary(pseud.gfit.bm)
plot(pseud.gfit.bm, main="Histogram under H0; Brownian Motion")

set.seed(42)
pseud.gfit.comp = gfit(target=stat.pseud[1,],
                         sumstat=traits[models=="comp",],
                         statistic=mean,
                         nb.replicate=100)

summary(pseud.gfit.comp)
plot(pseud.gfit.comp, main="Histogram under H0; Competition")

set.seed(42)
pseud.gfit.comp_selec = gfit(target=stat.pseud[1,],
                               sumstat=traits[models=="comp_selec",],
                               statistic=mean,
                               nb.replicate=100)

summary(pseud.gfit.comp_selec)
plot(pseud.gfit.comp_selec, main="Histogram under H0; Competition + Selection")
...
for (i in unique(models)){
  pseud.kde <- kde2d(res.prcomp$x[which(models == i),1], res.prcomp$x[which(models == i),2])
  dx <- diff(pseud.kde$x[1:2])  # lifted from emdbook::HPDregionplot()
  dy <- diff(pseud.kde$y[1:2])
  sz <- sort(pseud.kde$z)
  c1 <- cumsum(sz) * dx * dy
  dimnames(pseud.kde$z) <- list(pseud.kde$x, pseud.kde$y)
  dc[[i]] <- reshape2::melt(pseud.kde$z)
...
ggplot(bind_rows(dc),aes(x=Var1,y=Var2))+
  ...
  geom_point(aes(x = predict(res.prcomp, stat.pseud[1,])[1],
                 y = predict(res.prcomp, stat.pseud[1,])[2]), colour = "darkred", size = 1.5) +
  theme_bw() +
...
# cross-validation
stat.selec.sim <- subset(traits,subset=models=="bm")
par.selec.sim <- subset(traits_models[,1:7],subset=models=="bm")
...
set.seed(42)
res_params <- abc(target=stat.pseud[1,],
                  param=data.frame(sigma2=par.selec.sim[, "bm"],
                                   lambda1=par.selec.sim[, "lambda1"],
                                   tau0=par.selec.sim[, "tau0"],
                                   mubg=par.selec.sim[, "mubg"],
                                   muibg=par.selec.sim[, "muibg"]),
...
# simulate:
post_params <- res_params$unadj.values %>% as_tibble() %>% group_by(sigma2, lambda1, tau0, mubg, muibg) %>% count() %>% rename(rep = n)

mapply(runCAMPSITE.ABC, post_params$rep, post_params$lambda1, post_params$tau0, post_params$mubg, post_params$muibg, post_params$sigma2, rep(0, dim(post_params)[1]), rep(0, dim(post_params)[1]), "output/emp_ppc/pseuds/")

# summarise
models <- list.files("output/emp_ppc/pseuds", full.names = T)
model.names <- list.files("output/emp_ppc/pseuds", full.names = F)
...
ppc_models %>% write_csv("output/summary_pseuds_empirical_ppc.csv")

ppc_models <- read_csv("output/summary_pseuds_empirical_ppc.csv")
