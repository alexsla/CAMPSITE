...
### anoles ###
anole_data <- read_csv("empirical/data/Anoles.csv") %>% as.data.frame()
anole_tree <- read.tree("empirical/trees/Anoles.tre")

rownames(anole_data) <- anole_data$species

sortedData <- sortTraitData(phy = anole_tree, y = anole_data, data.name = "trait", log.trait = F)

anole_age <- round(max(nodeHeights(anole_tree)), 0)

anole_root <- transformPhylo.ML(y = sortedData$trait, phy = sortedData$phy, model = "bm")$root.state

# estimate BM
anole_bm <- transformPhylo.ML(y = sortedData$trait, phy = sortedData$phy, model = "bm")$brownianVariance[[1]]

# estimate OU
anole_ou <- transformPhylo.ML(y = sortedData$trait, phy = sortedData$phy, model = "ou")$Alpha[[1]]

# estimate MC
anole_mc <- abs(fit_t_comp(sortedData$phy, setNames(as.numeric(t(sortedData$trait)), rownames(sortedData$trait)), model = "MC")$s)
...
sim.vals <- tibble(lambda1 = sapply(round(abs(rnorm(100, r$b, sd = 0.01)), 3), max, 0.001),
                   ...
                   bm.vals = sapply(round(abs(rnorm(100, anole_bm, sd = 0.05)), 3), max, 0.001),
                   mc.vals = sapply(round(abs(rnorm(100, anole_mc, sd = 0.05)), 3), max, 0.001),
                   ou.vals = sapply(round(abs(rnorm(100, anole_ou, sd = 0.05)), 3), max, 0.001)) %>%
  unique()
...
runCAMPSITE.ABC <- function(nsim, lambda1, tau0, mubg, muibg, bm, mc, ou, path){
  ...
  set.seed(42)
  sim <- foreach(i=1:nsim, .packages = c("ape"), .export = c("anole_root", "anole_age")) %dopar% {
    repeat {
      model <- sim_CAMPSITE(pars = c(lambda1,
                                     ...
                                     bm,
                                     20),
                            ou = list(c(anole_root),
                                      c(ou),
                                      c(ou)),
                            bounds = c(-Inf,
                                       Inf),
                            root.value = anole_root,
                            age.max = anole_age,
                            step.size = 0.01,
                            full.sim = T,
                            plot = F
...
# simulate "BM":
mapply(runCAMPSITE.ABC, rep(100, 100), sim.vals$lambda1, sim.vals$tau0, sim.vals$mubg, sim.vals$muibg, sim.vals$bm.vals, rep(0, 100), rep(0, 100), rep("output/emp_sims/anoles/", 100))

# simulate competition:
mapply(runCAMPSITE.ABC, rep(100, 100), sim.vals$lambda1, sim.vals$tau0, sim.vals$mubg, sim.vals$muibg, sim.vals$bm.vals, sim.vals$mc.vals, rep(0, 100), rep("output/emp_sims/anoles/", 100))

# simulate selection:
mapply(runCAMPSITE.ABC, rep(100, 100), sim.vals$lambda1, sim.vals$tau0, sim.vals$mubg, sim.vals$muibg, sim.vals$bm.vals, rep(0, 100), sim.vals$ou.vals, rep("output/emp_sims/anoles/", 100))

# simulate competition+selection:
mapply(runCAMPSITE.ABC, rep(100, 100), sim.vals$lambda1, sim.vals$tau0, sim.vals$mubg, sim.vals$muibg, sim.vals$bm.vals, sim.vals$mc.vals, sim.vals$ou.vals, rep("output/emp_sims/anoles/", 100))

# summarise
models <- list.files("output/emp_sims/anoles", full.names = T)
model.names <- list.files("output/emp_sims/anoles", full.names = F)
...
for (i in 1:length(results)){
  MNND_multi[[i]] <- as.tibble(matrix(sapply(results[[i]], "[[", "MNND"), ncol = 100)) %>%
    add_column(time = c(1:(anole_age*100)),
...
traits_models %>% write_csv("output/summary_anoles_empirical_models.csv")

traits_models <- read_csv("output/summary_anoles_empirical_models.csv") %>% as.data.frame()
...
# model selection
stat.anole <- data.frame(sd = sd(sortedData$trait),
...
set.seed(42)
modsel.anole <- postpr(stat.anole[1,],
                         models,
                         traits,
                         tol=.2,
                         method="mnlogistic")

summary(modsel.anole)

# goodness-of-fit

set.seed(42)
anole.gfit.selec = gfit(target=stat.anole[1,],
                          sumstat=traits[models=="selec",],
                          statistic=mean,
                          nb.replicate=100)

summary(anole.gfit.selec)
plot(anole.gfit.selec, main="Histogram under H0; Selection")

set.seed(42)
anole.gfit.bm = gfit(target=stat.anole[1,],
                       sumstat=traits[models=="bm",],
                       statistic=mean,
                       nb.replicate=100)

summary(anole.gfit.bm)
plot(anole.gfit.bm, main="Histogram under H0; Brownian Motion")

set.seed(42)
anole.gfit.comp = gfit(target=stat.anole[1,],
                         sumstat=traits[models=="comp",],
                         statistic=mean,
                         nb.replicate=100)

summary(anole.gfit.comp)
plot(anole.gfit.comp, main="Histogram under H0; Competition")

set.seed(42)
anole.gfit.comp_selec = gfit(target=stat.anole[1,],
                               sumstat=traits[models=="comp_selec",],
                               statistic=mean,
                               nb.replicate=100)

summary(anole.gfit.comp_selec)
plot(anole.gfit.comp_selec, main="Histogram under H0; Competition + Selection")
...
for (i in unique(models)){
  anole.kde <- kde2d(res.prcomp$x[which(models == i),1], res.prcomp$x[which(models == i),2])
  dx <- diff(anole.kde$x[1:2])  # lifted from emdbook::HPDregionplot()
  dy <- diff(anole.kde$y[1:2])
  sz <- sort(anole.kde$z)
  c1 <- cumsum(sz) * dx * dy
  dimnames(anole.kde$z) <- list(anole.kde$x, anole.kde$y)
  dc[[i]] <- reshape2::melt(anole.kde$z)
...
ggplot(bind_rows(dc),aes(x=Var1,y=Var2))+
  ...
  geom_point(aes(x = predict(res.prcomp, stat.anole[1,])[1],
                 y = predict(res.prcomp, stat.anole[1,])[2]), colour = "darkred", size = 1.5) +
  theme_bw() +
...
set.seed(42)
res_params <- abc(target=stat.anole[1,],
...
mapply(runCAMPSITE.ABC, post_params$rep, post_params$lambda1, post_params$tau0, post_params$mubg, post_params$muibg, post_params$sigma2, rep(0, dim(post_params)[1]), rep(0, dim(post_params)[1]), "output/emp_ppc/anoles/")

# summarise
models <- list.files("output/emp_ppc/anoles", full.names = T)
model.names <- list.files("output/emp_ppc/anoles", full.names = F)
...
ppc_models %>% write_csv("output/summary_anoles_empirical_ppc.csv")

ppc_models <- read_csv("output/summary_anoles_empirical_ppc.csv")
