...
### SKINKS ###
skink_data <- read_csv("empirical/data/Sphenomorphinae.csv") %>% as.data.frame()
skink_tree <- read.tree("empirical/trees/Sphenomorphinae.tre")

rownames(skink_data) <- skink_data$species

sortedData <- sortTraitData(phy = skink_tree, y = skink_data, data.name = "trait", log.trait = F)

skink_age <- round(max(nodeHeights(skink_tree)), 0)

skink_root <- transformPhylo.ML(y = sortedData$trait, phy = sortedData$phy, model = "bm")$root.state

# estimate BM
skink_bm <- transformPhylo.ML(y = sortedData$trait, phy = sortedData$phy, model = "bm")$brownianVariance[[1]]

# estimate OU
skink_ou <- transformPhylo.ML(y = sortedData$trait, phy = sortedData$phy, model = "ou")$Alpha[[1]]

# estimate MC
skink_mc <- abs(fit_t_comp(sortedData$phy, setNames(as.numeric(t(sortedData$trait)), rownames(sortedData$trait)), model = "MC")$s)
...
sim.vals <- tibble(lambda1 = sapply(round(abs(rnorm(100, r$b, sd = 0.01)), 3), max, 0.001),
                   ...
                   bm.vals = sapply(round(abs(rnorm(100, skink_bm*100, sd = 0.05)), 3), max, 0.001),
                   mc.vals = sapply(round(abs(rnorm(100, skink_mc, sd = 0.05)), 3), max, 0.001),
                   ou.vals = sapply(round(abs(rnorm(100, skink_ou, sd = 0.05)), 3), max, 0.001)) %>%
  unique()
...
runCAMPSITE.ABC <- function(nsim, lambda1, tau0, mubg, muibg, bm, mc, ou, path){
  ...
  set.seed(42)
  sim <- foreach(i=1:nsim, .packages = c("ape"), .export = c("skink_root", "skink_age")) %dopar% {
    repeat {
      model <- sim_CAMPSITE(pars = c(lambda1,
                                     ...
                                     bm,
                                     20),
                            ou = list(c(skink_root),
                                      c(ou),
                                      c(ou)),
                            bounds = c(-Inf,
                                       Inf),
                            root.value = skink_root,
                            age.max = skink_age,
                            step.size = 0.01,
                            full.sim = T,
                            plot = F
...
# 
# # simulate "BM":
# mapply(runCAMPSITE.ABC, rep(100, 100), sim.vals$lambda1, sim.vals$tau0, sim.vals$mubg, sim.vals$muibg, sim.vals$bm.vals, rep(0, 100), rep(0, 100), rep("output/emp_sims/skinks/", 100))
# 
# # simulate competition:
# mapply(runCAMPSITE.ABC, rep(100, 100), sim.vals$lambda1, sim.vals$tau0, sim.vals$mubg, sim.vals$muibg, sim.vals$bm.vals, sim.vals$mc.vals, rep(0, 100), rep("output/emp_sims/skinks/", 100))
# 
# # simulate selection:
# mapply(runCAMPSITE.ABC, rep(100, 100), sim.vals$lambda1, sim.vals$tau0, sim.vals$mubg, sim.vals$muibg, sim.vals$bm.vals, rep(0, 100), sim.vals$ou.vals, rep("output/emp_sims/skinks/", 100))
# 
# # simulate competition+selection:
# mapply(runCAMPSITE.ABC, rep(100, 100), sim.vals$lambda1, sim.vals$tau0, sim.vals$mubg, sim.vals$muibg, sim.vals$bm.vals, sim.vals$mc.vals, sim.vals$ou.vals, rep("output/emp_sims/skinks/", 100))
#
# summarise
models <- list.files("output/emp_sims/skinks", full.names = T)
model.names <- list.files("output/emp_sims/skinks", full.names = F)
# 
# cores = detectCores()
# cl <- makeCluster(cores[1]-1) #not to overload your computer
# registerDoParallel(cl)
...
#                            T ~ "comp_selec"))
# 
# traits_models %>% write_csv("output/summary_skinks_empirical_models.csv")

traits_models <- read_csv("output/summary_skinks_empirical_models.csv") %>% as.data.frame()
...
# model selection
stat.skink <- data.frame(sd = sd(sortedData$trait),
...
set.seed(42)
modsel.skink <- postpr(stat.skink[1,],
                       models,
                       traits,
                       tol=.025,
                       method="mnlogistic")

summary(modsel.skink)

# goodness-of-fit

set.seed(42)
skink.gfit.selec = gfit(target=stat.skink[1,],
                        sumstat=traits[models=="selec",],
                        statistic=mean,
                        nb.replicate=100)

summary(skink.gfit.selec)
plot(skink.gfit.selec, main="Histogram under H0; Selection")

set.seed(42)
skink.gfit.bm = gfit(target=stat.skink[1,],
                        sumstat=traits[models=="bm",],
                        statistic=mean,
                        nb.replicate=100)

summary(skink.gfit.bm)
plot(skink.gfit.bm, main="Histogram under H0; Brownian Motion")

set.seed(42)
skink.gfit.comp = gfit(target=stat.skink[1,],
                        sumstat=traits[models=="comp",],
                        statistic=mean,
                        nb.replicate=100)

summary(skink.gfit.comp)
plot(skink.gfit.comp, main="Histogram under H0; Competition")

set.seed(42)
skink.gfit.comp_selec = gfit(target=stat.skink[1,],
                        sumstat=traits[models=="comp_selec",],
                        statistic=mean,
                        nb.replicate=100)

summary(skink.gfit.comp_selec)
plot(skink.gfit.comp_selec, main="Histogram under H0; Competition + Selection")
...
for (i in unique(models)){
  skink.kde <- kde2d(res.prcomp$x[which(models == i),1], res.prcomp$x[which(models == i),2])
  dx <- diff(skink.kde$x[1:2])  # lifted from emdbook::HPDregionplot()
  dy <- diff(skink.kde$y[1:2])
  sz <- sort(skink.kde$z)
  c1 <- cumsum(sz) * dx * dy
  dimnames(skink.kde$z) <- list(skink.kde$x, skink.kde$y)
  dc[[i]] <- reshape2::melt(skink.kde$z)
...
ggplot(bind_rows(dc),aes(x=Var1,y=Var2))+
  ...
  geom_point(aes(x = predict(res.prcomp, stat.skink[1,])[1],
                 y = predict(res.prcomp, stat.skink[1,])[2]), colour = "darkred", size = 1.5) +
  theme_bw() +
...
set.seed(42)
res_params <- abc(target=stat.skink[1,],
...
# 
# mapply(runCAMPSITE.ABC, post_params$rep, post_params$lambda1, post_params$tau0, post_params$mubg, post_params$muibg, post_params$sigma2, rep(0, dim(post_params)[1]), post_params$alpha, "output/emp_ppc/skinks/")
# 
# # summarise
# models <- list.files("output/emp_ppc/skinks", full.names = T)
# model.names <- list.files("output/emp_ppc/skinks", full.names = F)
# 
# cores = detectCores()
# cl <- makeCluster(cores[1]-1) #not to overload your computer
# registerDoParallel(cl)
...
#               rename(richness = value))
# 
# ppc_models %>% write_csv("output/summary_skinks_empirical_ppc.csv")

ppc_models <- read_csv("output/summary_skinks_empirical_ppc.csv")
