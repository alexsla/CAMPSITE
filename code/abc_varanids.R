...
### varanids ###
varanid_data <- read_csv("empirical/data/Varanidae.csv") %>% as.data.frame()
varanid_tree <- read.tree("empirical/trees/Varanidae.tre")

rownames(varanid_data) <- varanid_data$species

sortedData <- sortTraitData(phy = varanid_tree, y = varanid_data, data.name = "trait", log.trait = F)

varanid_age <- round(max(nodeHeights(varanid_tree)), 0)

varanid_root <- transformPhylo.ML(y = sortedData$trait, phy = sortedData$phy, model = "bm")$root.state

# estimate BM
varanid_bm <- transformPhylo.ML(y = sortedData$trait, phy = sortedData$phy, model = "bm")$brownianVariance[[1]]

# estimate OU
varanid_ou <- transformPhylo.ML(y = sortedData$trait, phy = sortedData$phy, model = "ou")$Alpha[[1]]

# estimate MC
varanid_mc <- abs(fit_t_comp(sortedData$phy, setNames(as.numeric(t(sortedData$trait)), rownames(sortedData$trait)), model = "MC")$s)
...
sim.vals <- tibble(lambda1 = sapply(round(abs(rnorm(100, r$b, sd = 0.01)), 3), max, 0.001),
                   ...
                   bm.vals = sapply(round(abs(rnorm(100, varanid_bm, sd = 0.05)), 3), max, 0.001),
                   mc.vals = sapply(round(abs(rnorm(100, varanid_mc, sd = 0.05)), 3), max, 0.001),
                   ou.vals = sapply(round(abs(rnorm(100, varanid_ou, sd = 0.05)), 3), max, 0.001)) %>%
  unique()
...
runCAMPSITE.ABC <- function(nsim, lambda1, tau0, mubg, muibg, bm, mc, ou, path){
  ...
  set.seed(42)
  sim <- foreach(i=1:nsim, .packages = c("ape"), .export = c("varanid_root", "varanid_age")) %dopar% {
    repeat {
      model <- sim_CAMPSITE(pars = c(lambda1,
                                     ...
                                     bm,
                                     20),
                            ou = list(c(varanid_root),
                                      c(ou),
                                      c(ou)),
                            bounds = c(-Inf,
                                       Inf),
                            root.value = varanid_root,
                            age.max = varanid_age,
                            step.size = 0.01,
                            full.sim = T,
                            plot = F
...
# simulate "BM":
mapply(runCAMPSITE.ABC, rep(100, 100), sim.vals$lambda1, sim.vals$tau0, sim.vals$mubg, sim.vals$muibg, sim.vals$bm.vals, rep(0, 100), rep(0, 100), rep("output/emp_sims/varanids/", 100))

# simulate competition:
mapply(runCAMPSITE.ABC, rep(100, 100), sim.vals$lambda1, sim.vals$tau0, sim.vals$mubg, sim.vals$muibg, sim.vals$bm.vals, sim.vals$mc.vals, rep(0, 100), rep("output/emp_sims/varanids/", 100))

# simulate selection:
mapply(runCAMPSITE.ABC, rep(100, 100), sim.vals$lambda1, sim.vals$tau0, sim.vals$mubg, sim.vals$muibg, sim.vals$bm.vals, rep(0, 100), sim.vals$ou.vals, rep("output/emp_sims/varanids/", 100))

# simulate competition+selection:
mapply(runCAMPSITE.ABC, rep(100, 100), sim.vals$lambda1, sim.vals$tau0, sim.vals$mubg, sim.vals$muibg, sim.vals$bm.vals, sim.vals$mc.vals, sim.vals$ou.vals, rep("output/emp_sims/varanids/", 100))

# summarise
models <- list.files("output/emp_sims/varanids", full.names = T)
model.names <- list.files("output/emp_sims/varanids", full.names = F)
...
traits_models %>% write_csv("output/summary_varanids_empirical_models.csv")

traits_models <- read_csv("output/summary_varanids_empirical_models.csv") %>% as.data.frame()
...
# model selection
stat.varanid <- data.frame(sd = sd(sortedData$trait),
...
set.seed(42)
modsel.varanid <- postpr(stat.varanid[1,],
                       models,
                       traits,
                       tol=.2,
                       method="mnlogistic")

summary(modsel.varanid)

# goodness-of-fit

set.seed(42)
varanid.gfit.selec = gfit(target=stat.varanid[1,],
                        sumstat=traits[models=="selec",],
                        statistic=mean,
                        nb.replicate=100)

summary(varanid.gfit.selec)
plot(varanid.gfit.selec, main="Histogram under H0; Selection")

set.seed(42)
varanid.gfit.bm = gfit(target=stat.varanid[1,],
                     sumstat=traits[models=="bm",],
                     statistic=mean,
                     nb.replicate=100)

summary(varanid.gfit.bm)
plot(varanid.gfit.bm, main="Histogram under H0; Brownian Motion")

set.seed(42)
varanid.gfit.comp = gfit(target=stat.varanid[1,],
                       sumstat=traits[models=="comp",],
                       statistic=mean,
                       nb.replicate=100)

summary(varanid.gfit.comp)
plot(varanid.gfit.comp, main="Histogram under H0; Competition")

set.seed(42)
varanid.gfit.comp_selec = gfit(target=stat.varanid[1,],
                             sumstat=traits[models=="comp_selec",],
                             statistic=mean,
                             nb.replicate=100)

summary(varanid.gfit.comp_selec)
plot(varanid.gfit.comp_selec, main="Histogram under H0; Competition + Selection")
...
for (i in unique(models)){
  varanid.kde <- kde2d(res.prcomp$x[which(models == i),1], res.prcomp$x[which(models == i),2])
  dx <- diff(varanid.kde$x[1:2])  # lifted from emdbook::HPDregionplot()
  dy <- diff(varanid.kde$y[1:2])
  sz <- sort(varanid.kde$z)
  c1 <- cumsum(sz) * dx * dy
  dimnames(varanid.kde$z) <- list(varanid.kde$x, varanid.kde$y)
  dc[[i]] <- reshape2::melt(varanid.kde$z)
...
ggplot(bind_rows(dc),aes(x=Var1,y=Var2))+
  ...
  geom_point(aes(x = predict(res.prcomp, stat.varanid[1,])[1],
                 y = predict(res.prcomp, stat.varanid[1,])[2]), colour = "darkred", size = 1.5) +
  theme_bw() +
...
set.seed(42)
res_params <- abc(target=stat.varanid[1,],
...
mapply(runCAMPSITE.ABC, post_params$rep, post_params$lambda1, post_params$tau0, post_params$mubg, post_params$muibg, post_params$sigma2, rep(0, dim(post_params)[1]), post_params$alpha, "output/emp_ppc/varanids/")

# summarise
models <- list.files("output/emp_ppc/varanids", full.names = T)
model.names <- list.files("output/emp_ppc/varanids", full.names = F)
...
ppc_models %>% write_csv("output/summary_varanids_empirical_ppc.csv")

ppc_models <- read_csv("output/summary_varanids_empirical_ppc.csv")
