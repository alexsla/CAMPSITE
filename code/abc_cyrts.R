...
### cyrts ###
cyrt_data <- read_csv("empirical/data/Cyrtodactylus.csv") %>% as.data.frame()
cyrt_tree <- read.tree("empirical/trees/Cyrtodactylus.tre")

rownames(cyrt_data) <- cyrt_data$species

sortedData <- sortTraitData(phy = cyrt_tree, y = cyrt_data, data.name = "trait", log.trait = F)

cyrt_age <- round(max(nodeHeights(cyrt_tree)), 0)

cyrt_root <- transformPhylo.ML(y = sortedData$trait, phy = sortedData$phy, model = "bm")$root.state

# estimate BM
cyrt_bm <- transformPhylo.ML(y = sortedData$trait, phy = sortedData$phy, model = "bm")$brownianVariance[[1]]

# estimate OU
cyrt_ou <- transformPhylo.ML(y = sortedData$trait, phy = sortedData$phy, model = "ou")$Alpha[[1]]

# estimate MC
cyrt_mc <- abs(fit_t_comp(sortedData$phy, setNames(as.numeric(t(sortedData$trait)), rownames(sortedData$trait)), model = "MC")$s)
...
sim.vals <- tibble(lambda1 = sapply(round(abs(rnorm(100, r$b, sd = 0.01)), 3), max, 0.001),
                   ...
                   bm.vals = sapply(round(abs(rnorm(100, cyrt_bm, sd = 0.05)), 3), max, 0.001),
                   mc.vals = sapply(round(abs(rnorm(100, cyrt_mc, sd = 0.05)), 3), max, 0.001),
                   ou.vals = sapply(round(abs(rnorm(100, cyrt_ou, sd = 0.05)), 3), max, 0.001)) %>%
  unique()
...
runCAMPSITE.ABC <- function(nsim, lambda1, tau0, mubg, muibg, bm, mc, ou, path){
  ...
  set.seed(42)
  sim <- foreach(i=1:nsim, .packages = c("ape"), .export = c("cyrt_root", "cyrt_age")) %dopar% {
    repeat {
...
# simulate "BM":
mapply(runCAMPSITE.ABC, rep(100, 100), sim.vals$lambda1, sim.vals$tau0, sim.vals$mubg, sim.vals$muibg, sim.vals$bm.vals, rep(0, 100), rep(0, 100), rep("output/emp_sims/cyrts/", 100))

# simulate competition:
mapply(runCAMPSITE.ABC, rep(100, 100), sim.vals$lambda1, sim.vals$tau0, sim.vals$mubg, sim.vals$muibg, sim.vals$bm.vals, sim.vals$mc.vals, rep(0, 100), rep("output/emp_sims/cyrts/", 100))

# simulate selection:
mapply(runCAMPSITE.ABC, rep(100, 100), sim.vals$lambda1, sim.vals$tau0, sim.vals$mubg, sim.vals$muibg, sim.vals$bm.vals, rep(0, 100), sim.vals$ou.vals, rep("output/emp_sims/cyrts/", 100))

# simulate competition+selection:
mapply(runCAMPSITE.ABC, rep(100, 100), sim.vals$lambda1, sim.vals$tau0, sim.vals$mubg, sim.vals$muibg, sim.vals$bm.vals, sim.vals$mc.vals, sim.vals$ou.vals, rep("output/emp_sims/cyrts/", 100))

# summarise
models <- list.files("output/emp_sims/cyrts", full.names = T)
model.names <- list.files("output/emp_sims/cyrts", full.names = F)
...
traits_models %>% write_csv("output/summary_cyrts_empirical_models.csv")

traits_models <- read_csv("output/summary_cyrts_empirical_models.csv") %>% as.data.frame()
...
# model selection
stat.cyrt <- data.frame(sd = sd(sortedData$trait),
...
set.seed(42)
modsel.cyrt <- postpr(stat.cyrt[1,],
                         models,
                         traits,
                         tol=.025,
                         method="mnlogistic")
...
set.seed(42)
cyrt.gfit.selec = gfit(target=stat.cyrt[1,],
                          sumstat=traits[models=="selec",],
                          statistic=mean,
                          nb.replicate=100)

summary(cyrt.gfit.selec)
plot(cyrt.gfit.selec, main="Histogram under H0; Selection")

set.seed(42)
cyrt.gfit.bm = gfit(target=stat.cyrt[1,],
                       sumstat=traits[models=="bm",],
                       statistic=mean,
                       nb.replicate=100)

summary(cyrt.gfit.bm)
plot(cyrt.gfit.bm, main="Histogram under H0; Brownian Motion")

set.seed(42)
cyrt.gfit.comp = gfit(target=stat.cyrt[1,],
                         sumstat=traits[models=="comp",],
                         statistic=mean,
                         nb.replicate=100)

summary(cyrt.gfit.comp)
plot(cyrt.gfit.comp, main="Histogram under H0; Competition")

set.seed(42)
cyrt.gfit.comp_selec = gfit(target=stat.cyrt[1,],
                               sumstat=traits[models=="comp_selec",],
                               statistic=mean,
                               nb.replicate=100)

summary(cyrt.gfit.comp_selec)
plot(cyrt.gfit.comp_selec, main="Histogram under H0; Competition + Selection")
...
for (i in unique(models)){
  cyrt.kde <- kde2d(res.prcomp$x[which(models == i),1], res.prcomp$x[which(models == i),2])
  dx <- diff(cyrt.kde$x[1:2])  # lifted from emdbook::HPDregionplot()
  dy <- diff(cyrt.kde$y[1:2])
  sz <- sort(cyrt.kde$z)
  c1 <- cumsum(sz) * dx * dy
  dimnames(cyrt.kde$z) <- list(cyrt.kde$x, cyrt.kde$y)
  dc[[i]] <- reshape2::melt(cyrt.kde$z)
...
ggplot(bind_rows(dc),aes(x=Var1,y=Var2))+
  ...
  geom_point(aes(x = predict(res.prcomp, stat.cyrt[1,])[1],
                 y = predict(res.prcomp, stat.cyrt[1,])[2]), colour = "darkred", size = 1.5) +
  theme_bw() +
...
set.seed(42)
res_params <- abc(target=stat.cyrt[1,],
...
mapply(runCAMPSITE.ABC, post_params$rep, post_params$lambda1, post_params$tau0, post_params$mubg, post_params$muibg, post_params$sigma2, rep(0, dim(post_params)[1]), post_params$alpha, "output/emp_ppc/cyrts/")

# summarise
models <- list.files("output/emp_ppc/cyrts", full.names = T)
model.names <- list.files("output/emp_ppc/cyrts", full.names = F)
...
ppc_models %>% write_csv("output/summary_cyrts_empirical_ppc.csv")

ppc_models <- read_csv("output/summary_cyrts_empirical_ppc.csv")
