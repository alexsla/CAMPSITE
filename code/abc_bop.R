...
### BoP ###
bop_data <- read_csv("empirical/data/BoP.csv") %>% as.data.frame()
bop_tree <- read.tree("empirical/trees/BoP.tre") %>%
  keep.tip(bop_data$species)

rownames(bop_data) <- bop_data$species

sortedData <- sortTraitData(phy = bop_tree, y = bop_data, data.name = "trait", log.trait = F)

bop_age <- round(max(nodeHeights(bop_tree)), 0)

bop_root <- transformPhylo.ML(y = sortedData$trait, phy = sortedData$phy, model = "bm")$root.state

# estimate BM
bop_bm <- transformPhylo.ML(y = sortedData$trait, phy = sortedData$phy, model = "bm")$brownianVariance[[1]]

# estimate OU
bop_ou <- transformPhylo.ML(y = sortedData$trait, phy = sortedData$phy, model = "ou")$Alpha[[1]]

# estimate MC
bop_mc <- abs(fit_t_comp(sortedData$phy, setNames(as.numeric(t(sortedData$trait)), rownames(sortedData$trait)), model = "MC")$s)
...
sim.vals <- tibble(lambda1 = sapply(round(abs(rnorm(100, r$b, sd = 0.01)), 3), max, 0.001),
                   ...
                   bm.vals = sapply(round(abs(rnorm(100, bop_bm/10, sd = 0.05)), 3), max, 0.001),
                   mc.vals = sapply(round(abs(rnorm(100, bop_mc, sd = 0.05)), 3), max, 0.001),
                   ou.vals = sapply(round(abs(rnorm(100, bop_ou, sd = 0.05)), 3), max, 0.001)) %>%
  unique()
...
runCAMPSITE.ABC <- function(nsim, lambda1, tau0, mubg, muibg, bm, mc, ou, path){
  ...
  set.seed(42)
  sim <- foreach(i=1:nsim, .packages = c("ape"), .export = c("bop_root", "bop_age")) %dopar% {
    repeat {
...
# 
# # simulate "BM":
# mapply(runCAMPSITE.ABC, rep(100, 100), sim.vals$lambda1, sim.vals$tau0, sim.vals$mubg, sim.vals$muibg, sim.vals$bm.vals, rep(0, 100), rep(0, 100), rep("output/emp_sims/bop/", 100))
# 
# # simulate competition:
# mapply(runCAMPSITE.ABC, rep(100, 100), sim.vals$lambda1, sim.vals$tau0, sim.vals$mubg, sim.vals$muibg, sim.vals$bm.vals, sim.vals$mc.vals, rep(0, 100), rep("output/emp_sims/bop/", 100))
# 
# # simulate selection:
# mapply(runCAMPSITE.ABC, rep(100, 100), sim.vals$lambda1, sim.vals$tau0, sim.vals$mubg, sim.vals$muibg, sim.vals$bm.vals, rep(0, 100), sim.vals$ou.vals, rep("output/emp_sims/bop/", 100))
# 
# # simulate competition+selection:
# mapply(runCAMPSITE.ABC, rep(100, 100), sim.vals$lambda1, sim.vals$tau0, sim.vals$mubg, sim.vals$muibg, sim.vals$bm.vals, sim.vals$mc.vals, sim.vals$ou.vals, rep("output/emp_sims/bop/", 100))
# 
# summarise
models <- list.files("output/emp_sims/bop", full.names = T)
model.names <- list.files("output/emp_sims/bop", full.names = F)
# 
# cores = detectCores()
# cl <- makeCluster(cores[1]-1) #not to overload your computer
# registerDoParallel(cl)
...
#                            T ~ "comp_selec"))
# 
# traits_models %>% write_csv("output/summary_bop_empirical_models.csv")

traits_models <- read_csv("output/summary_bop_empirical_models.csv") %>% as.data.frame()
...
stat.bop <- data.frame(sd = sd(sortedData$trait),
...
set.seed(42)
bop.gfit.selec = gfit(target=stat.bop[1,],
                      sumstat=traits[models=="selec",],
                      statistic=mean,
                      nb.replicate=100)

summary(bop.gfit.selec)
plot(bop.gfit.selec, main="Histogram under H0; Selection")

set.seed(42)
bop.gfit.bm = gfit(target=stat.bop[1,],
                   sumstat=traits[models=="bm",],
                   statistic=mean,
                   nb.replicate=100)

summary(bop.gfit.bm)
plot(bop.gfit.bm, main="Histogram under H0; Brownian Motion")

set.seed(42)
bop.gfit.comp = gfit(target=stat.bop[1,],
                     sumstat=traits[models=="comp",],
                     statistic=mean,
                     nb.replicate=100)

summary(bop.gfit.comp)
plot(bop.gfit.comp, main="Histogram under H0; Competition")

set.seed(42)
bop.gfit.comp_selec = gfit(target=stat.bop[1,],
                           sumstat=traits[models=="comp_selec",],
                           statistic=mean,
                           nb.replicate=100)

summary(bop.gfit.comp_selec)
plot(bop.gfit.comp_selec, main="Histogram under H0; Competition + Selection")
...
for (i in unique(models)){
  bop.kde <- kde2d(res.prcomp$x[which(models == i),1], res.prcomp$x[which(models == i),2])
  dx <- diff(bop.kde$x[1:2])  # lifted from emdbook::HPDregionplot()
  dy <- diff(bop.kde$y[1:2])
  sz <- sort(bop.kde$z)
  c1 <- cumsum(sz) * dx * dy
  dimnames(bop.kde$z) <- list(bop.kde$x, bop.kde$y)
  dc[[i]] <- reshape2::melt(bop.kde$z)
...
ggplot(bind_rows(dc),aes(x=Var1,y=Var2))+
  ...
  geom_point(aes(x = predict(res.prcomp, stat.bop[1,])[1],
                 y = predict(res.prcomp, stat.bop[1,])[2]), colour = "darkred", size = 1.5) +
  theme_bw() +
...
# cross-validation
stat.selec.sim <- subset(traits,subset=models=="comp_selec")
par.selec.sim <- subset(traits_models[,1:7],subset=models=="comp_selec")

set.seed(42)
cv.res.rej <- cv4abc(data.frame(alpha=par.selec.sim[, "selection"],
                                s=par.selec.sim[, "competition"],
...
set.seed(42)
res_params <- abc(target=stat.bop[1,],
...
# simulate:
post_params <- res_params$unadj.values %>% as_tibble() %>% group_by(s, alpha, sigma2, lambda1, tau0, mubg, muibg) %>% count() %>% rename(rep = n)

mapply(runCAMPSITE.ABC, post_params$rep, post_params$lambda1, post_params$tau0, post_params$mubg, post_params$muibg, post_params$sigma2, post_params$s, post_params$alpha, "output/emp_ppc/bop/")

# summarise
models <- list.files("output/emp_ppc/bop", full.names = T)
model.names <- list.files("output/emp_ppc/bop", full.names = F)
...
ppc_models %>% write_csv("output/summary_bop_empirical_ppc.csv")

ppc_models <- read_csv("output/summary_bop_empirical_ppc.csv")
